...
def getBreathCount(filename, savefile=''):
	...
	while(cap.isOpened()):
		...
		if (frameId % math.floor(frameRate)==0):	#берем каждый 3й кадр
			...
			for (x, y, w, h) in faces:
				...
				print(below_face, below_shoulder, left_from_face, right_from_face)
				if x>700: # проверяем на присутствие контуров на изображение
					break
				frame = frame[below_face:below_shoulder, left_from_face:right_from_face]
				ZeroFrame = np.zeros((below_shoulder-below_face),(right_from_face-left_from_face))
				if frame == ZeroFrame:
					break
	...
	output = {'breaths': len(breath_count[0])}
	if (savefile != ''):
		with open(savefile, 'w') as json_file:
			json.dump(output, json_file)

	return new_signal, output

if __name__ == '__main__':
	new_signal, output = getBreathCount('test5.mp4', savefile='test.json')
	print('количество вдохов', output['breaths'])
